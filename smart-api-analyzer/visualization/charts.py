"""Visualization layer — all Plotly chart builders."""
...
def language_pie(lang_series: pd.Series) -> go.Figure:
    ...
    fig.update_layout(showlegend=True, margin=dict(t=50, b=20))
    return fig


def repo_language_bar(lang_series: pd.Series) -> go.Figure:
    ...
    fig.update_layout(coloraxis_showscale=False, margin=dict(t=50, b=20))
    return fig


def commit_trend(df: pd.DataFrame) -> go.Figure:
    fig = px.area(
        df,
        x="week",
        y="commits",
...
def stars_scatter(df: pd.DataFrame) -> go.Figure:
    fig = px.scatter(
        df,
        x="forks_count",
        y="stargazers_count",
...
def activity_bar(df: pd.DataFrame) -> go.Figure:
    fig = px.bar(
        df,
        x="activity_score",
        y="name",
    ...
    fig.update_layout(yaxis=dict(autorange="reversed"), coloraxis_showscale=False, margin=dict(t=50, b=20))
    return fig


# ── Weather Charts ────────────────────────────────────────────────────────────

def temperature_range_chart(df: pd.DataFrame) -> go.Figure:
    fig = go.Figure([
...
def precipitation_chart(df: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    fig.add_bar(
        x=df["day_label"], y=df["precipitation"],
...
def hourly_temperature_chart(df: pd.DataFrame) -> go.Figure:
    # Show next 48 hours only
    df48 = df.head(48)
    fig = go.Figure()
    fig.add_scatter(
...
def wind_chart(df: pd.DataFrame) -> go.Figure:
    fig = px.bar(
...
def uv_index_chart(df: pd.DataFrame) -> go.Figure:
    colors = []
    for v in df["uv_index"]:
        if v is None:
            colors.append("#aaa")
        elif v >= 11:
            colors.append("#7B0000")
        elif v >= 8:
...
def historical_temp_trend(df: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    fig.add_scatter(x=df["date"], y=df["temp_max"], mode="lines", name="Max Temp", line=dict(color="#EF553B"))
    fig.add_scatter(x=df["date"], y=df["temp_min"], mode="lines", name="Min Temp", line=dict(color="#636EFA"))
    fig.add_scatter(
